...
public class RecalculateScoreExecutor implements Runnable {
	...
	public void run() {
        ...
        logger.info("recalculation of score of survey {} started", surveyid);
		try {
			...
			for (Integer surveyId : surveyIds) {
                ...
                while (results.next()) {
                    AnswerSet answerSet = (AnswerSet)results.get();
        ...
        logger.info("recalculation of score of survey {} finished", surveyid);
	}
}
