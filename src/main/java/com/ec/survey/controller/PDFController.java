...
import com.ec.survey.handler.worker.SurveyExecutor;
...
public class PDFController extends BasicController {
	...
	public ModelAndView pubsurvey(@PathVariable String id, HttpServletRequest request, Locale locale,
			HttpServletResponse response) throws ForbiddenURLException, InvalidURLException {
		try {
			...
			if (survey.getIsDraft()) {
				Survey published = surveyService.getSurvey(survey.getUniqueId(), false, false, false, false,
						survey.getLanguage().getCode(), true, false);
	...
	public ModelAndView survey(@PathVariable String id, HttpServletRequest request, Locale locale,
			HttpServletResponse response) throws ForbiddenURLException, InvalidURLException {
		try {
	...
	private void SendFile(Survey survey, HttpServletRequest request, HttpServletResponse response, String language) throws IOException {
		...
		if (survey.getIsDraft()) {
			value = "attachment;filename=" + survey.getShortname() + "_"
					+ Tools.formatDate(new Date(), ConversionTools.DateFormat) + "_" + language + "_draft.pdf";
		} else {
			value = "attachment;filename=" + survey.getShortname() + "_"
					+ Tools.formatDate(survey.getUpdated(), ConversionTools.DateFormat) + "_" + language + ".pdf";
		}
		...
		java.io.File result = new java.io.File(
				String.format("%s/survey%s%s.pdf", folder.getPath(), survey.getId(), lang));
		...
		try {
			...
			// ClientAbortException can happen if the user closes the browser during
			// download
			// FileNotFoundException can happen as we create the files asynchronously and it
			// might not have been created yet
		}
	}

	@RequestMapping(value = "/surveyexists/{id}", method = { RequestMethod.GET, RequestMethod.HEAD })
	public @ResponseBody String surveyexists(@PathVariable String id, HttpServletRequest request, Locale locale,
	...
	public @ResponseBody String surveyready(@PathVariable String id, HttpServletRequest request, Locale locale,
			HttpServletResponse response) {
		try {
			String lang = request.getParameter("lang");
	...
	public @ResponseBody String answerexists(@PathVariable String code, HttpServletRequest request, Locale locale,
			HttpServletResponse response) {
		try {
	...
	public @ResponseBody String answerready(@PathVariable String code, HttpServletRequest request, Locale locale,
			HttpServletResponse response) {
		try {
			java.io.File target = new java.io.File(String.format("%sanswer%s.pdf", tempFileDir, code));
			if (target.exists() && target.length() > 0) {
	...
	public ModelAndView answer(@PathVariable String code, HttpServletRequest request, Locale locale,
			HttpServletResponse response) throws InvalidURLException, NotAgreedToTosException, ForbiddenURLException,
			WeakAuthenticationException, NotAgreedToPsException {
		...
		try {
			...
			java.io.File result = new java.io.File(String.format("%sanswer%s.pdf", tempFileDir, code));
			...
			if (result.exists()) {
				...
				response.setHeader("Content-Disposition",
						"attachment;filename=" + answerSet.getSurvey().getShortname() + "_" + code + ".pdf");
				response.setContentType("application/pdf");
