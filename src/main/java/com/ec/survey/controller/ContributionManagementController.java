...
public class ContributionManagementController extends BasicController {
	...
	public ModelAndView contributionsearch(HttpServletRequest request, Model model) throws Exception {
		...
		if (request.getParameter("keepfilter") != null) {
			...
			if (request.getParameter("itemsPerPage") != null)
				ConversionTools.getInt(request.getParameter("itemsPerPage"), 50);
	...
	public @ResponseBody List<ContributionSearchResult> contributionsearchJSON(HttpServletRequest request) {
		try {
			...
			for (AnswerSet answerSet : answerSets) {
				result.add(new ContributionSearchResult(answerSet.getSurvey().getUniqueId(),
						answerSet.getSurvey().getShortname(), answerSet.getSurvey().cleanTitle(),
						answerSet.getDraftId(), answerSet.getUniqueCode(), answerSet.getIsDraft(),
						answerSet.getInvitationId(), answerSet.getUpdateDate()));
			}
			return result;
		} catch (Exception e) {
	...
	public @ResponseBody String resetcontribution(Locale locale, HttpServletRequest request)
			throws NotAgreedToTosException, WeakAuthenticationException, NotAgreedToPsException, MessageException {
