...
import com.ec.survey.handler.worker.AttendeeUpdater;
...
public class AddressBookController extends BasicController {
	...
	public ModelAndView attendees(HttpServletRequest request) throws Exception {
		...
		} else if (request.getParameter(Constants.EDITED) != null && !request.getParameter(Constants.EDITED).isEmpty())
		{
		...
		} else if (request.getParameter(Constants.DELETED) != null && !request.getParameter(Constants.DELETED).isEmpty()) {
	...
	public @ResponseBody String checkNewAttendee(HttpServletRequest request, HttpServletResponse response ) throws Exception {
		...
		if (user.getGlobalPrivileges().get(GlobalPrivilege.UserManagement) > 1 && !ownerlogin.isEmpty())
		{
			...
			} else if (entry.getKey().equals("owner"))
			{
				...
				if (owner != null && !owner.trim().isEmpty())
				{
					try {
						...
						return new ModelAndView(Constants.VIEW_ERROR_GENERIC, Constants.MESSAGE, Objects.requireNonNull(resources.getMessage("error.OwnerNotValid", null, "The selected owner is not a valid user", locale)));
					}
				}
					...
					try {
						...
						try (CsvListReader reader = new CsvListReader(new InputStreamReader(inputStream), preference)) {
							...
							if (hasHeaderRow) {
								...
								if (fileheaders[fileheaders.length - 1].trim().isEmpty()) {
							...
							while ((row = reader.read()) != null) {
								String[] a = removeFormulas(row.toArray(new String[0]));

								rows.add(a);
								if (fileheaders == null) {
									fileheaders = row.toArray(new String[0]);
								}
							}
						}
						inputStream.close();
						...
						if (numrows == 0)
						{
							...
							for (int r = start; r < numrows; r++) {
								...
								for (int i = 0; i < numcells; i++)
								{
									...
									if (empty && !values[i].isEmpty()) empty = false;
								}
								if (!empty)
								{
									rows.add(values);
	...
	private static String getText(Sheet sheet, int row, int cell)
	{
		...
		try {
			...
			if (String.valueOf(sheet.getRow(row).getCell(cell).getCellType()).equals(String.valueOf(CellType.NUMERIC))){
	...
	public ModelAndView importAttendeesCheck(HttpServletRequest request, Locale locale) throws Exception {
		...
		for (String[] row: rows)
		{
			...
			if (attendee.getName() == null || attendee.getName().trim().isEmpty())
			{
				...
				valid.add(false);
				existing.add(false);
			} else if (attendee.getEmail() == null || attendee.getEmail().trim().isEmpty() || !MailService.isValidEmailAddress(attendee.getEmail().trim()))
			{
	...
	public ModelAndView attendeesPOST(HttpServletRequest request, Locale locale) throws Exception {
		...
		{
            switch (target) {
                case "importAttendeesCheck" -> {
                    return importAttendeesCheck(request, locale);
                }
                case "importAttendees2" -> {
	...
	public String add(HttpServletRequest request) throws NotAgreedToTosException, WeakAuthenticationException, NotAgreedToPsException, MessageException {
		...
		for (Entry<String, String[]> entry : parameterMap.entrySet()) {
			...
			if (entry.getKey().equalsIgnoreCase("name")) {
				...
				if (user.getGlobalPrivileges().get(GlobalPrivilege.UserManagement) > 1 && !entry.getValue()[0].isEmpty())
				{
		...
		for (Entry<String, String> entry : keysMap.entrySet()) {
			if (!entry.getValue().trim().isEmpty() && valuesMap.containsKey(entry.getKey()))
			{
