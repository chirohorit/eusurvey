...
public class InvitationMailCreator implements Runnable {
	...
	protected static final Logger logger = LoggerFactory.getLogger(InvitationMailCreator.class);
	...
	public void run() {
		...
		try {
			...
			if (task.getSelectedAttendee() != null && !task.getSelectedAttendee().trim().isEmpty()) {
	...
	private String insertDisclaimer(Survey survey, String body) {
		//disclaimer
		if(survey.getOwner().getType().equals("SYSTEM")) {
			body += "<br/><br/>";
