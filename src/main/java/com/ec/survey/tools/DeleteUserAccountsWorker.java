...
public class DeleteUserAccountsWorker implements Runnable {
    ...
    protected static final Logger logger = LoggerFactory.getLogger(DeleteUserAccountsWorker.class);
    ...
    public void run() {
        ...
        for (int userid : userids) {
            try {
                ...
                //Deletion of all the user's webservice tasks
                List<WebserviceTask> webserviceTasks = webserviceService.getTasksForUser(administrationService.getUser(userid));
                for (WebserviceTask webserviceTask : webserviceTasks) {
                    webserviceService.delete(webserviceTask);
                }

                //Deletion of all the user's invitation email templates
                List<InvitationTemplate> templates = participationService.getTemplates(userid);
                for (InvitationTemplate template : templates) {
                    participationService.delete(template);
                }
